...
bool GMSurveyAnswersRec::Read(SFile* f, const char* stringBuffer) {
	uint32_t tempAnswer_langIndices[17];
	if (
		...
		SFile::Read(f, &tempAnswer_langIndices[16], sizeof(uint32_t), nullptr, nullptr, nullptr) == 0
	)
	{
	...
	if (stringBuffer) {
		...
		m_Answer_lang[16] = &stringBuffer[tempAnswer_langIndices[16]];
		return true;
	}
	return true;
}
